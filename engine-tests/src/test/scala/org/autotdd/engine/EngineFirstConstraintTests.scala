...
class EngineFirstConstraintTests extends EngineStringStringTests {
  ...
  "An empty engine" should "change from root to if then with one constraint" in {
    ...
    assertEngineMatches(engine, Right(Node(because = "A", inputs = List("A"), yes = Left(CodeAndConstraints("X", List(scenario))), no = Left(CodeAndConstraints("Z")), constraintThatCausedNode = scenario)))
  ...
  it should "Add constraint to root if adding assertion" in {
    ...
    val engine = Engine1("Z", UseCase("", scenario))
    assertEngineMatches(engine, Left(CodeAndConstraints("Z", List(scenario))))
  }
