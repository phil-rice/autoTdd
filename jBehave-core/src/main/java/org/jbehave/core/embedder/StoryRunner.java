...
/**
 * Runs a {@link Story}, given a {@link Configuration} and a list of {@link CandidateSteps}, describing the results to the {@link StoryReporter}.
 * 
...
public class StoryRunner {
	...
	Map<Story, StoryDuration> cancelledStories = new HashMap<Story, StoryDuration>();

	/**
	 * Run steps before or after a collection of stories. Steps are execute only <b>once</b> per collection of stories.
	 * 
	 * @param configuration
	 *            the Configuration used to find the steps to run
	 * @param candidateSteps
	 *            the List of CandidateSteps containing the candidate steps methods
	 * @param stage
	 *            the Stage
	...
	public State runBeforeOrAfterStories(Configuration configuration, List<CandidateSteps> candidateSteps, Stage stage) {
		...
		RunContext context = new RunContext(this, configuration, candidateSteps, storyPath, MetaFilter.EMPTY);
	 ...
	 *            the Filter to apply to the story Meta
	 * @param beforeStories
	 *            the State before running any of the stories, if not <code>null</code>
	 * @throws Throwable
	 *             if failures occurred and FailureStrategy dictates it to be re-thrown.
	 */
	 ...
	 *            the Configuration used to run story
	 * @param stepsFactory
	 *            the InjectableStepsFactory used to created the candidate steps methods
	 * @param story
	 *            the Story to run
	 * @param filter
	 *            the Filter to apply to the story Meta
	...
	public void run(Configuration configuration, InjectableStepsFactory stepsFactory, Story story, MetaFilter filter, State beforeStories) throws Throwable {
		RunContext context = new RunContext(this, configuration, stepsFactory, story.getPath(), filter);
	 ...
	 *            the story text
	 * @param storyId
	 *            the story Id, which will be returned as story path
	 * @return The parsed Story
	 */
	...
	private void runScenariosParametrisedByExamples(RunContext context, Scenario scenario, Meta storyAndScenarioMeta) throws Throwable {
	...
	private void runBeforeOrAfterScenarioSteps(RunContext context, Scenario scenario, Meta storyAndScenarioMeta, Stage stage, ScenarioType type) throws InterruptedException {
	...
	final class FineSoFar implements State {
