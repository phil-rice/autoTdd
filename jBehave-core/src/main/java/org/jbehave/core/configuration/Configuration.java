...
/**
 * <p>
 * Provides the configuration used by the {@link Embedder} and the in the {@link Embeddable} implementations to customise its runtime properties.
 * </p>
 * <p>
 * Configuration implements a <a href="http://en.wikipedia.org/wiki/Builder_pattern">Builder</a> pattern so that each element of the configuration can be specified individually, and read well. All elements have default values, which can be overridden by the "use" methods. The "use" methods allow to override the dependencies one by one and play nicer with a Java hierarchical structure, in that does allow the use of non-static member variables.
 * </p>
 */
public abstract class Configuration {
	 ...
	 * <p/>
	 * If there are multiple scenarios in a single story, this could cause the story to stop after the first failing scenario.
	 * <p/>
	 ...
	 * <p/>
	 * Uses wanting a stricter behaviour for pending steps may use {@link FailingUponPendingStep}.
	 */
	...
	/**
	 * Silent monitoring that does not produce any noise of the step matching. </p> If needed, users can switch on verbose monitoring using {@link PrintStreamStepMonitor}
	 */
