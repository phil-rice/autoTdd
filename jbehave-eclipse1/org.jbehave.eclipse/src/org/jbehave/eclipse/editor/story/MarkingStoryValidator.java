...
public class MarkingStoryValidator {
	...
	private void checkNarrative(final fj.data.List<Part> parts) throws JavaModelException {
		...
		while (iterator.hasNext()) {
			...
			} else if (keyword != Keyword.Ignorable && keyword != Keyword.Meta && keyword != Keyword.MetaProperty && keyword != Keyword.GivenStories) {
	...
	private void checkSteps(final fj.data.List<Part> parts) throws JavaModelException {
		...
		System.out.println("Validatein states: " + org.softwarefm.utilities.strings.Strings.join(Iterables.list(steps), "\n"));
		...
		System.out.println("Candidates evaluated: " + org.softwarefm.utilities.strings.Strings.join(steps, "\n"));
		...
		for (Part part : steps) {
			...
			System.out.println("  part: " + key + "\n    " + part + "\n    Candidates(" + candidates.size() + "):" + Iterables.list(candidates));
			...
			if (count == 0)
				...
				if (countWithMax > 1) {
					...
					MarkData mark = part.addInfoMark(Marks.Code.MultipleMatchingSteps_PrioritySelection, "Multiple steps matching, but only one with the highest priority for <" + key + ">");
					Marks.putStepsAsHtml(mark, candidates);
	...
	class Part {
		...
		public void evaluateCandidate(StepCandidate candidate) {
			...
			if (typeMatch)
				System.out.println("eval " + this + "\n  " + candidate + "\n   patternMatch: " + patternMatch + ", typeMatch: " + typeMatch);
		...
		public String toString() {
			return "Part [offset=" + storyElement.getOffset() + ", length=" + storyElement.getLength() + ", keyword=" + storyElement.getPreferredKeyword() + ", marks=" + marks + ", text=" + textWithoutTrailingNewlines() + "]";
		}
