...
public class StepHyperLinkDetector implements IHyperlinkDetector {
	...
	public IHyperlink[] detectHyperlinks(final ITextViewer viewer, final IRegion region, boolean canShowMultipleHyperlinks) {
		...
		final Ref<StoryElement> found = new StoryDocumentUtils(localizedStepSupport).findStoryElementAtRegion(document, region);
		if (found.isNull()) {
		...
		IHyperlink link = new IHyperlink() {
			...
			public void open() {
				try {
					StepJumper stepJumper = new StepJumper(jbehaveProject);
					stepJumper.jumpToDeclaration(viewer, step);
				} catch (Exception e) {
