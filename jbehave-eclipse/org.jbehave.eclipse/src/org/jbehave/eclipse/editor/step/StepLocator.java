...
public class StepLocator {
	 ...
	 * 
	 * When 'Bob' clicks on the 'login' button When 'Bob' clicks on the ... When 'Bo ...
	 * 
	 */
	public Iterable<WeightedStep> findCandidatesStartingWith(final String step) {
		...
		try {
			...
			final String searchedType = StepSupport.stepType(localizedStepSupport, step);
			final String stepWithoutKeyword = StepSupport.stepWithoutKeyword(localizedStepSupport, step);
			...
			log.debug("Candidates starting with <{}> found: #{}", step, elements.size());
			return elements;
	...
	/**
	 * Returns the first {@link StepCandidate} found that match the step, ordered by priority. Be careful that there can be several other {@link StepCandidate}s that fulfill the step too.
	 * 
	 * @param step
	 * @return
	 */
	public StepCandidate findFirstStep(final String step) {
		...
		try {
			...
			StepCandidate found = getFirstStepWithHighestPrio(matchingStepVisitor.getElementsFound());
			if (found == null) {
	 ...
	 * @return
	 */
	private StepCandidate getFirstStepWithHighestPrio(Iterable<StepCandidate> candidates) {
		List<StepCandidate> collectedPrios = Lists.sort(candidates, new Comparator<StepCandidate>() {
			@Override
			public int compare(StepCandidate o1, StepCandidate o2) {
				if (o1.priority != o2.priority)
					return o2.priority - o1.priority;
				return o2.stepPattern.length() - o1.stepPattern.length(); // heuristic more information in a longer step pattern
			}
		});
		if (collectedPrios.isEmpty())
			return null;
		else
			return collectedPrios.get(0);
	}
