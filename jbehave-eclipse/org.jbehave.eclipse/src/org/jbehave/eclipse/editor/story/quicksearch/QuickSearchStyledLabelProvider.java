...
public class QuickSearchStyledLabelProvider extends StyledCellLabelProvider implements TextProvider {
	...
	/**
	 * @param imageRegistry
	 *            used to retrieve the label's image
	...
	private void defineText(StepCandidate pStep, ViewerCell cell) {
		...
		if (displayDecoration) {
			String decoration = MessageFormat.format(" ({0}#{1})", new Object[] { pStep.method.getParent().getElementName(), pStep.method.getElementName() }); //$NON-NLS-1$
