...
public class JBehaveProject implements StepCandidateCacheListener {
	...
	public JBehaveProject(IProject project) {
		...
		this.cacheLoader = new StepCandidateCacheLoader(this, Activator.getDefault().getExecutor(), JBehaveProject.getSystemJobAsExecutor("JBehave cache refresh task"));
	...
	protected void initializeClassScannerPreferencesAndListener(IProject project) {
		...
		this.classScannerPreferences.addListener(new IPreferenceChangeListener() {
			...
			public void preferenceChange(PreferenceChangeEvent changeEvent) {
				log.info("Class scanner preference changed [{}]: <{}> -> <{}>", o(changeEvent.getKey(), changeEvent.getOldValue(), changeEvent.getNewValue()));
	...
	protected void initializeProjectPreferencesAndListener(IProject project) {
		...
		this.projectPreferences.addListener(new IPreferenceChangeListener() {
			@Override
			public void preferenceChange(PreferenceChangeEvent changeEvent) {
				log.info("Project preference changed [{}]: <{}> -> <{}>", o(changeEvent.getKey(), changeEvent.getOldValue(), changeEvent.getNewValue()));
	...
	private Callback<IMethod, Container<StepCandidate>> newCallback() {
		return new Callback<IMethod, Container<StepCandidate>>() {
			public void op(IMethod method, final Container<StepCandidate> container) {
				...
				MethodToStepCandidateReducer reducer = new MethodToStepCandidateReducer();
	...
	private StepCandidateReduceListener getStepCandidateReduceListener(final Container<StepCandidate> container) {
		return new StepCandidateReduceListener() {

			@Override
			public void add(IMethod method, StepType stepType, String stepPattern, Integer priority) {
				container.add(new StepCandidate(getLocalizedStepSupport(), parameterPrefix, method, stepType, stepPattern, priority));
			}
		};
	};
