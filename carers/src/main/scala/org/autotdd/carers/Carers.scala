...
object Carers {
  ...
  val hasQualifyingBenefit = Engine.stm[World, String, Boolean]().
    ...
    scenario(World.blankTestWorld, "CL100104A", "Data present in database").
    expected(true).

    scenario(World.blankTestWorld, "CL100106A", "Data not present in database").
    expected(false).
    because((i: InTxn, w: World, nino: String) => {
      w.ninoToDecision(i, w, nino) != <noDecision/>
    });

  //so how do I do 'calculate the expenses... in many ways this is a sum if exists... Multiple true and inheritance were very good here
  //

  //  Options.. a different detector for each each type
  ...
  val engine = Engine[World, Elem, ReasonAndAmount]().
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100110A"), "CL100110A-child care allowance").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100111A"), "CL100111A-private pension").
    expected(ReasonAndAmount("carers.validClaim", Some(95))).
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100115A"), "CL115A-sub let").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100112A"), "CL100112A-occupational pension").
    expected(ReasonAndAmount("carers.validClaim", Some(95))).
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100104A"), "Cl100104A-Age Under 16").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100105A"), "CL100105A-lessThen35Hours").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100107A"), "CL100107A-notInGB").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100108A"), "CL100108A-restriction on immigration status").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100109A"), "CL100109A-full time education").
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100113A"), "CL100113A-paid employment earning too much").
    expected(ReasonAndAmount("carers.emploment.moreThan100PerWeek", None)).
    because((w: World, e: Elem) => {
      val expenses: Double = calculateExpenses(w, e).amount.getOrElse(0);
      val weeklyIncome: Double = calculateWeeklyIncome(w, e)
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100114A"), "CL114A-self employed earning too much").
    expected(ReasonAndAmount("carers.emploment.moreThan100PerWeek", None)).
    ...
    scenario(World.blankTestWorld, Xmls.validateClaim("CL100117A"), "CL100117A-self employed earning too much").
  ...
  def main(args: Array[String]) {
    ...
    val sharedWord = World.blankTestWorld
