...
public class AutoTddJUnitDescriptionGenerator {
	...
	public AutoTddJUnitDescriptionGenerator(List<CandidateSteps> candidateSteps, Configuration configuration) {
	...
	private void insertDescriptionForExamples(Scenario scenario, Description scenarioDescription) {
		...
		for (Map<String, String> row : rows) {
			Description exampleRowDescription = Description.createSuiteDescription(EXAMPLE_DESCRIPTION_PREFIX + row, (Annotation[]) null);
	...
	private void addPendingStep(Description description, String stringStep) {
		testCases++;
		Description testDescription = Description.createSuiteDescription(getJunitSafeString("[PENDING] " + stringStep));
	...
	private void addRegularStep(Description description, String stringStep, StepCandidate step) {
		...
		// the correct class.
		Description testDescription = Description.createTestDescription(step.getStepsInstance().getClass(), getStepName(stringStep, step));
		description.addChild(testDescription);
	}

	private String getStepName(String stringStep, StepCandidate step) {
//		Method method = step.getMethod();
//		if (method == null)
			return getJunitSafeString(stringStep);
		// else
	...
	private void addCompositeSteps(Description description, String stringStep, StepCandidate step) {
		...
		testDescription = Description.createSuiteDescription(getStepName(stringStep, step));
